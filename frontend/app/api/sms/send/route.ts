...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('SMS API received:', { to, message, from });

    // Validate input
    if (!to || !Array.isArray(to) || to.length === 0) {
      console.log('Validation failed: to field invalid', { to });
      return NextResponse.json(
        { success: false, error: 'Recipients (to) must be a non-empty array' },
        { status: 400 }
      );
    }
