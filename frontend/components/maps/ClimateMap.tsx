  ...
  useEffect(() => {
    const fetchCounties = async () => {
      try {
        ...
        // Transform backend data to include climate data
        const countiesWithClimateData = data.counties.map((county: any) => ({
          ...
          currentTemp: county.climate?.currentTemp || 22,
          rainfall: county.climate?.rainfall || 50,
          humidity: county.climate?.humidity || 60,
          ndvi: county.climate?.ndvi || 0.5,
          alerts: county.climate?.alerts || 0
        }))
        ...
        setError('Failed to load county data. Using fallback data.')
